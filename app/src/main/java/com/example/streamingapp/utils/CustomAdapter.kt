...
interface CheckListActions<T> {
     ...
     * @param afterUpdatingList Provide function which will be executed after list update and it will also provide the selected item as parameter.
     */
    fun checkLikeRadioButton(
        whichItemShouldBeChecked: (T) -> Boolean,
        afterUpdatingList: (selectedItem: T) -> Unit = {}
    )
...
class CustomAdapter<T : Any, B : androidx.databinding.ViewDataBinding>(
    ...
    override fun checkLikeRadioButton(
        whichItemShouldBeChecked: (T) -> Boolean,
        afterUpdatingList: (selectedItem: T) -> Unit
    ) {
